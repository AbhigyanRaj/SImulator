...
{
    if (SIZE > 32 || SIZE < 1)
    {                                               //functions of logic gates hai yaha pe!!
        perror("Invalid List");
        exit(EXIT_FAILURE);
    }
    Bit* result = createBitArray(SIZE);
...
Bit* SUB(Bit* BIT_ARRAY_1, Bit* BIT_ARRAY_2, unsigned char INPUT_ARRAY_SIZE, unsigned char OUTPUT_ARRAY_SIZE) {
    ...
    {
        perror("Invalid List");   // agar 32 bit sa jada ho gyaa or 1 sai kaam huaa tho error dega!!
        exit(EXIT_FAILURE);    
    }
...
Bit* toUnsignedBitArray(int decimal, unsigned char SIZE)
{
    ...
    int i = SIZE - 1;
    while (i >= 0)                              //converting in unsigned binaray number ...
    {
...
Bit* toSignedBitArray(int decimal, unsigned char SIZE)
{
    if (SIZE > 32 || SIZE < 1)
    {
        ...
        exit(EXIT_FAILURE);
                                                               ...
                                                               //converting in signed binaray number ...
    Bit* bitArray = createBitArray(SIZE);
...
int toDecimal(const Bit* BIT_ARRAY, unsigned char ARRAY_START, unsigned char ARRAY_END, bool IS_SIGNED)
{
    ...
    while (i < ARRAY_END)
    {
        ...
        ++i;                                                //yeh convert karega binaray ko decimal mai..
    }
...
short signExtend(const Bit* BIT_ARRAY, unsigned char SIZE)
{
    ...
    while (i < SIZE)
    {
        temporary[32 - SIZE + i] = BIT_ARRAY[i];      //this is sign extension (Computer organisation wala topic)
        ++i;
    }
    int i = 0;
    while (i < 32 - SIZE)
